...
export function CountryPicker({
  ...
  const Row = ({ id, label, code }: { id: string; label: string; code?: string }) => (
    <button
      type="button"
      onClick={() => {
        onChange(id);
      ...
      className={`flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-[11px] transition-colors hover:bg-[#0e1420] ${
        id === value ? "bg-[#0f1826] text-txt" : "text-dim"
      }`}
    >
            ...
            <>
              ...
              >
                ...
                <div className="max-h-[280px] overflow-y-auto">
                  <Row id="" label="All countries" />
                  {filtered.map((c) => (
                    <Row key={c._id} id={c._id} label={c.name} code={c.code.toUpperCase()} />
                  ))}
                </div>
              </div>
            </>,
