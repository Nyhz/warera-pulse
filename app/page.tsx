  ...
  return (
    <div className="mx-auto flex w-full max-w-[1920px] flex-col lg:h-dvh lg:overflow-hidden">
      ...
      <main className="flex flex-1 flex-col gap-px bg-line md:grid md:grid-cols-[200px_minmax(0,1fr)] lg:min-h-0 lg:grid-cols-[248px_minmax(0,1fr)_360px] lg:[grid-template-rows:minmax(0,5fr)_minmax(0,2fr)]">
        ...
        {/* Hot nations + active conflicts: bottom row at md, right column at lg */}
        <div className="order-3 grid min-w-0 grid-cols-1 gap-px bg-line md:grid-cols-2 md:[grid-column:1/3] md:[grid-row:3] lg:grid-cols-1 lg:grid-rows-[auto_minmax(0,1fr)] lg:[grid-column:3] lg:[grid-row:1/3] lg:min-h-0">
          <HotNations className="shrink-0 md:self-start lg:self-auto" />
          <Conflicts className="md:max-h-[460px] lg:max-h-none lg:min-h-0" />
        </div>

        {/* Global feed — last on mobile */}
        <Feed className="order-4 h-[420px] min-w-0 md:[grid-column:2] md:[grid-row:2] lg:h-auto lg:min-h-0" />
      </main>
